...
func MapGet[K comparable, V any](values map[K]V, key K) (V, bool) {
	value, ok := values[key]
	return value, ok
}

func MapSet[K comparable, V any](values map[K]V, key K, value V) {
	values[key] = value
}

func MapDelete[K comparable, V any](values map[K]V, key K) {
	delete(values, key)
}
